...
import math
from data_loader2_uncroppedscansonly import DataGenerator2   #for this classifier we import only DataGenerator2 since we are using the alpha angle
...
def Classify2_uncroppedScans():    #again specifiy the function to be 2 and hence refere to the alpha angle
  ...
  model.compile(optimizer=Adam(learning_rate=0.0007), loss='binary_crossentropy', metrics=['binary_accuracy', tf.keras.metrics.Recall(), specificity, tf.keras.metrics.AUC(), negative_predictive_value, positive_predictive_value, matthews_correlation_coefficient]) #very important line about model characteristics
  model.summary()
  ...
  checkpoint = ModelCheckpoint("results/classifier_uncroppedscans/saveweights/best_model" + file_time + ".hdf5", monitor='val_binary_accuracy', verbose=1,
  ...
  fig.savefig('results/classifier_uncroppedscans/classifier1_' + file_time + '.png')
  ...
  plt.subplots_adjust(top=0.85)
  plt.subplots_adjust(wspace=0.5, hspace=0.5)
  fig1.savefig('results/classifier_uncroppedscans/classifier2_' + file_time + '.png')
  ...
  plt.savefig('results/classifier_uncroppedscans/classifier_' + file_time + '.png')   #save graph
  plt.close()
  ...
  plt.savefig('results/classifier_uncroppedscans/loss_' + file_time + '.png') #save loss graph
  plt.close()
  ...
  plt.savefig('results/classifier_uncroppedscans/sensitivity_' + file_time + '.png')  #save sensitivity graph
  plt.close()
  ...
  plt.savefig('results/classifier_uncroppedscans/specificity_' + file_time + '.png')  #save specificity graph
  plt.close()
  ...
  plt.savefig('results/classifier_uncroppedscans/auc_' + file_time + '.png')  #save AUC graph
  plt.close()
  ...
  plt.savefig('results/classifier_uncroppedscans/npv_' + file_time + '.png')  #save NPV graph
  plt.close()
  ...
  plt.savefig('results/classifier_uncroppedscans/ppv_' + file_time + '.png')  #save PPV graph
  plt.close()
