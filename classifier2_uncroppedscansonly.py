...
def Classify2_uncroppedScans():    #again specifiy the function to be 2 and hence refere to the alpha angle
  ...
  model.compile(optimizer=Adam(learning_rate=0.008), loss='binary_crossentropy', metrics=['binary_accuracy', tf.keras.metrics.Recall(), specificity, tf.keras.metrics.AUC(), negative_predictive_value, positive_predictive_value, matthews_correlation_coefficient]) #very important line about model characteristics
  model.summary()
