...
def resnet2(HEIGHT, WIDTH, CHANNELS, NUM_OUTPUTS, NUM_SIDE, NUM_GENDER, NUM_INDICATION):
    ...
    z_patient = layers.Concatenate(axis = 1)([x, inputAlpha, inputBeta, inputBirthweight, inputSide, inputGender, inputIndication])
    ...
    model = keras.Model({"scan" : inputs, "gender" :inputGender, \
     "side" : inputSide, "indication" : inputIndication, "birthweight" : inputBirthweight, "alpha" : inputAlpha, \
     "beta" : inputBeta},  outputs) #[inputs, inputGender, inputSide, inputIndication, inputBirthweight, inputAlpha, inputBeta]
...
"""
def resnet2_Pre(HEIGHT, WIDTH, CHANNELS, NUM_OUTPUTS, NUM_SIDE, NUM_GENDER, NUM_INDICATION):
